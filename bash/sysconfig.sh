...
function error-message {
  echo "something is technically wrong" >&2
}
...
function error-exit {
  echo "something is technically wrong. :) $1"
  exit
}
#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {
  echo "OPTIONS available for this script are
-h --help, --os, --cpu, --disk, --host, --domain, --memory, --printer, --ipconfig
syntax: -
sysconfig.sh [option(s)]"
}
...
function cleanup {
  rm /tmp/*.$$
  echo "Script files deleted (Temporary Ones).. Exitiing Now"
  exit
}
trap cleanup SIGINT
